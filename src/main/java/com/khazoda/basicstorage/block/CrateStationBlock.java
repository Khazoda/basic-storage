...
public class CrateStationBlock extends BlockWithEntity implements BlockEntityProvider {
  public static final Settings defaultSettings = Settings.create().sounds(BlockSoundGroup.WOOD).strength(3.5f)
      .pistonBehavior(PistonBehavior.BLOCK).instrument(Instrument.BASS).mapColor(MapColor.OAK_TAN);
    ...
    UseBlockCallback.EVENT.register((PlayerEntity player, World world, Hand hand, BlockHitResult hit) -> {
      ...
      if (player.isSneaking()) {
        ...
        if (playerStack.isEmpty()) {
          if (!world.isClient())
            player.sendMessage(
                Text.translatable("message.basicstorage.station.connected_crate_count", connectedCrateCount).styled(s -> s.withColor(0xDDFF99)), true);
          return ActionResult.PASS;
        }
      ...
      if (!world.isClient()) {
        if (inserted <= 0) {
          player.sendMessage(Text.translatable("message.basicstorage.station.no_matching_crates").styled(s -> s.withColor(0xFF9999)), true);
  ...
  public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state,
    ...
    return checkType(type, BlockEntityRegistry.CRATE_STATION_BLOCK_ENTITY, CrateStationBlockEntity::tick);
  }
